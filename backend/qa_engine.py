...
class QAEngine:
    ...
    def find_answers(self, user_question: str, top_k: int = 3) -> list[dict]:
    ...
    def find_answer(self, user_question: str, min_confidence: float | None = None) -> dict:
      
        if min_confidence is None:
            min_confidence = self.min_confidence

        results = self.find_answers(user_question, top_k=3)
        best = results[0]
